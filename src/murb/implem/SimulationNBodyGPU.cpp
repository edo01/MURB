...
void SimulationNBodyGPU::finalize_iteration()
{
    // copy the accelerations from the device to the host
...
{   
    ...
    kernelComputeBodiesAcceleration<<<NTPB, NB>>>(this->getBodies().getDataAoS().data(), this->getBodies().getN(), 
                                                    this->softSquared, this->G, this->d_accelerations);
    this->finalize_iteration();
    // time integration
